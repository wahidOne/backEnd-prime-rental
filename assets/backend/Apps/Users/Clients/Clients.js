import { handleUIClients, configDataTableClients } from "./_handleUITable";
import { loadingModal } from "../../components/Modal-backEnd.service";
import { handleModalClientUI } from "./_handleModalClientUI";
...
export default class Clients {
	...
	loadData() {
		...
		getAllData.then((res) => {
			...
			if (this.table) {
				...
				arr.map(
					(client) =>
						(containertable += handleUIClients(this.userpath, client, no++))
				);
	...
	handleActionsTable(e) {
		if (
			e.target.id == "info-client" ||
			e.target.parentNode.id == "info-client"
		) {
			e.preventDefault();
			const clientId = e.target.dataset.id || e.target.parentNode.dataset.id;
			e.preventDefault();
			this.urlGetDetail =
				this.domain + "administrator/users/get-client/" + clientId;
			// getCostumer(urlGet, this.userpath);
			this.getdDetailData();
		}

		if (
			e.target.id == "delete-client" ||
			e.target.parentNode.id == "delete-client"
		) {
			e.preventDefault();

			const dataName =
				e.target.dataset.name || e.target.parentNode.dataset.name;
			const dataid = e.target.dataset.id || e.target.parentNode.dataset.id;
			const urlDelete =
				this.domain + "administrator/users/del-client/" + dataid;
		}
	}
	...
	getdDetailData() {
		...
		detaildata.then((res) => {
			...
			const containerModal = this.modalinfo.querySelector(
				"#container-modal-client"
			);

			const { client, transaction } = res.data;

			if (this.modalinfo) {
				loadingModal(this.modalinfo);
				let containerModalCLient = "";
				containerModalCLient += handleModalClientUI(
					this.modalinfo,
					client,
					this.userpath,
					this.domain,
